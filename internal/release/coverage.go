...
// new release.
//
// Staged for removal in plan/214 task 5. The `<?catalog?>`
// `row-expr:` parameter that this PR adds is the
// long-term replacement: the coverage page becomes a
// hand-authored skeleton with one `<?catalog?>` block per
// category, regenerated by `mdsmith fix` from the same
// front-matter blocks. This Go generator remains while two
// prep changes land (require `partial:` in the proto schema;
// line-break the row-expr source) so the page byte-content
// stays stable during the migration.
package release
