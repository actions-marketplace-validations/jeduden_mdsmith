...
func TestMessagingTargets_NoFragmentTargets(t *testing.T) {
	// The generated Markdown fragment layer is gone; every
	// tracked surface is now a structured non-Markdown file.
	for _, tg := range MessagingTargets("/r") {
		assert.NotContains(t, tg.Label, "fragment",
			"unexpected fragment target %q still registered", tg.Label)
		assert.NotContains(t, tg.Path, filepath.Join("docs", "brand", "fragments"),
			"target %q still points into the removed fragments directory", tg.Label)
	}
}
