...
That command table and the feature list above are generated by
mdsmith's own `<?catalog?>` and `<?include?>` directives.

Files can be paths, directories (walked for `*.md` / `*.markdown`),
or globs. Directories respect `.gitignore` (`--no-gitignore` to
override); explicitly named files are never filtered.

The [CLI reference](docs/reference/cli.md) covers shared flags, exit
codes, output format, and config merge semantics. Each subcommand
page above lists its own flags and examples.
