...
//
// Wired in via package.json's `bun test --preload ./src/test-setup.ts`
// — bun loads it once before any test file, so the mocks are in place
// before ESM hoists the import graph.
//
...
function decorate(node: FakeNode): FakeNode & {
  ...
  d.setAttribute = (name: string, value: string) => {
  ...
  const matches = (child: FakeNode, sel: string): boolean => {
    // Support only ".class" and "tag" selectors — the forms our
    // tests use.
    if (sel.startsWith(".")) {
      return child.className.split(/\s+/).includes(sel.slice(1));
    }
    return child.nodeName === sel.toUpperCase();
  };
  d.querySelector = (sel: string) => {
    for (const child of walk(node)) {
      if (matches(child, sel)) return decorate(child);
    }
    return null;
  };
  d.querySelectorAll = (sel: string) => {
    const out: Array<FakeNode & { click(): void }> = [];
    for (const child of walk(node)) {
      if (matches(child, sel)) out.push(decorate(child));
    }
    return out;
  };
  return d;
...
class PluginStub {
  ...
  removeCommand(_id: unknown): void {}
...
class ItemViewStub {
  containerEl: unknown = { children: [{}, docStub.createElement("div")] };
  leaf: unknown;
...
mock.module("@codemirror/state", () => ({
  StateEffect: {
    define: () => {
      const marker = { _marker: true };
      return {
        of: (v: unknown) => ({ value: v, is: (t: unknown) => t === marker }),
        is: (t: unknown) => t === marker,
        _marker: marker,
      };
    },
  },
