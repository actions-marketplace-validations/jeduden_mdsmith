...
func checkNoMisplacedDefault(file *syntax.File) error {
...
func visitForDefault(e syntax.Expr, disjunctionOperand bool) error {
	switch n := e.(type) {
	...
	case *syntax.Comprehension:
		// A comprehension is also an syntax.Decl; reuse the decl handler so the
...
func visitUnaryForDefault(n *syntax.UnaryExpr, disjunctionOperand bool) error {
	if n.Op == syntax.MUL {
...
func visitBinaryForDefault(n *syntax.BinaryExpr, _ bool) error {
	operand := n.Op == syntax.OR
...
func visitExprsForDefault(exprs []syntax.Expr) error {
	for _, e := range exprs {
...
// declarations.
func visitDeclsForDefault(decls []syntax.Decl) error {
	for _, d := range decls {
		if err := visitDeclForDefault(d); err != nil {
			return err
		}
...
// value.
func visitDeclForDefault(d syntax.Decl) error {
	switch n := d.(type) {
	case *syntax.Field:
	...
	case *syntax.Comprehension:
		if st, ok := n.Value.(*syntax.StructLit); ok {
...
func compileFile(file *syntax.File) (*engineValue, error) {
	if len(file.Decls) == 1 {
		if emb, ok := file.Decls[0].(*syntax.EmbedDecl); ok {
	...
	for _, d := range file.Decls {
		...
		out.fields = appendOrUnifyField(out.fields, field{
			name:     name,
			val:      val,
			optional: f.Constraint == syntax.OPTION,
		})
	}
...
// its value.
func compileExpr(e syntax.Expr) (*engineValue, error) {
...
func isDeferrable(e syntax.Expr) bool {
	switch n := e.(type) {
	case *syntax.ParenExpr:
		return isDeferrable(n.X)
	case *syntax.IndexExpr, *syntax.ListLit:
		return true
	case *syntax.BinaryExpr:
		switch n.Op {
		case syntax.GEQ, syntax.LEQ, syntax.GTR, syntax.LSS, syntax.NEQ, syntax.MAT, syntax.NMAT, syntax.EQL:
			return true
		}
	}
	return false
...
// compileBasicLit builds a concrete scalar from a literal syntax.
func compileBasicLit(n *syntax.BasicLit) (*engineValue, error) {
	switch n.Kind {
	case syntax.STRING:
...
// fieldLabel extracts the string name of a struct field label. The in-house
// syntax tree's Label is a closed interface — a bare identifier or a quoted
// string — so the two cases below are exhaustive; a definition, hidden, or
// type-keyword identifier label is rejected as outside the subset.
func fieldLabel(l syntax.Label) (string, error) {
	if bl, ok := l.(*syntax.BasicLit); ok {
		if bl.Kind != syntax.STRING {
			return "", fmt.Errorf("cuelite: unsupported field label %s", bl.Value)
		}
		s, err := syntax.Unquote(bl.Value)
		if err != nil {
			return "", fmt.Errorf("cuelite: field label %s: %w", bl.Value, err)
		}
		return s, nil
	}
	// The Label interface is closed to *Ident and *BasicLit; a non-BasicLit
	// label is therefore the *Ident arm.
	lab := l.(*syntax.Ident)
...
func compileUnary(n *syntax.UnaryExpr) (*engineValue, error) {
	switch n.Op {
	case syntax.GEQ, syntax.LEQ, syntax.GTR, syntax.LSS, syntax.NEQ, syntax.MAT, syntax.NMAT:
...
func boundOpOf(t syntax.Token) (boundOp, bool) {
	switch t {
	case syntax.GEQ:
		return opGe, true
	case syntax.LEQ:
...
func compileCall(n *syntax.CallExpr) (*engineValue, error) {
...
// in its error.
func compileSelectorCall(sel *syntax.SelectorExpr, args []syntax.Expr) (*engineValue, error) {
	pkg, ok := sel.X.(*syntax.Ident)
	if !ok {
	...
	// so sel.Sel is an *syntax.Ident; selName reads it directly.
...
// the *syntax.Ident directly; any other label node renders as "?" for an error
// message rather than failing.
func selectorName(l syntax.Label) string {
